...
float EVO_fitness(treenode_t *tree, bool render)
{
	...
	for (sun_x = 0; sun_x < SCREEN_WIDTH; sun_x += 10) {
		if (render) {
			GFX_Clear(GFX_WHITE);
			TREEGFX_draw(	tree,
					SCREEN_WIDTH / 2,
		...
		// TODO define sun radius differently
		// light on every leaf
		...
		for (i = 0; i < index; i++) {
			...
			for (q = 0; q < 9; q ++) {
				...
				for(l = 0; l < 5000 && !collision; l++) {
					px = lx + l *((float)sun_x - lx) / 5000;
					py = ly + l * ((float)sun_y - ly) / 5000;
	...
	return (float)subleafs_lit * pow(0.95, num_branches);
}
